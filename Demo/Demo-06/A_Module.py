...
Local Fast-Whisper STT, Pygame Audio Template Caching, and Zero-PII Dynamic gTTS Probing.
"""
...
class AudioEngine:
    def __init__(self):
        ...
        self.warmup() # Pre-warm audio kernels

    def warmup(self):
        """Forces CTranslate2 to pre-allocate memory and CUDA kernels."""
        if self.whisper_model is not None:
            try:
                # 0.1 seconds of silent PCM
                dummy_pcm = np.zeros(1600, dtype=np.float32)
                self.whisper_model.transcribe(dummy_pcm, beam_size=1)
                print("[+] Fast-Whisper audio buffers pre-warmed.")
            except Exception:
                pass
    ...
    # -------------------------------------------------------------------------
    # OFF-TOPIC / DYNAMIC PROBE MODE: Dynamic gTTS Synthesis for Zero-PII Prompts
    # -------------------------------------------------------------------------
    def speak_dynamic(self, text: str, lang: str = "th", block: bool = True):
        """
        Synthesizes dynamic follow-up questions via gTTS on the fly.
        Used strictly for system conversational steering & OPQRST clarification (Zero-PII guarantee).
        """
        print(f"[*] [Dynamic gTTS]: Synthesizing response -> \"{text}\"")
        def _worker():
            temp_path = None
            try:
    ...
    def detect_wake_word(utterance: str) -> tuple[bool, str]:
        """
        Checks if an ambient utterance contains the wake word.
