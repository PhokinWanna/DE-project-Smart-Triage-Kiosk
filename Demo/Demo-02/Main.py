...
# ก่อนเข้า Loop กล้อง ให้เปลี่ยนตัวแปรพวกนี้
CALIBRATION_TIME = 3.0
skin_calibrating = True
skin_calibration_start = 0
baseline_A = 0.0      # เปลี่ยนจาก baseline_S, baseline_H
A_history = []        # เปลี่ยนจาก S_history, H_history
    ...
    while cap.isOpened():
        ...
        if results.pose_landmarks:
            ...
            if r_cheek_roi.size != 0:
                # แปลงจาก BGR เป็น CIELab
                lab_roi = cv2.cvtColor(r_cheek_roi, cv2.COLOR_BGR2LAB)
                
                # ดึงเฉพาะแกน A (Red-Green) มาใช้ แกน L (ความสว่าง) เราไม่สนใจ
                avg_a = np.mean(lab_roi[:, :, 1]) 

                if skin_calibrating:
                    # เก็บค่า A ไว้ทำ Baseline
                    A_history.append(avg_a)
                    
                    if time.time() - skin_calibration_start > CALIBRATION_TIME:
                        skin_calibrating = False
                        baseline_A = np.mean(A_history)
                        print(f"[Skin Calibrated] Baseline A*: {baseline_A:.2f}")
                else:
                    # วิเคราะห์ความแตกต่างของแกนสีแดง
                    a_diff = avg_a - baseline_A 
                    
                    # ถ้า A ลดลงมาก แปลว่าความแดงหายไป (หน้าซีด)
                    if a_diff < -4.5:  
                        current_skin_status = "Pallor (Pale)"
                    # ถ้า A เพิ่มขึ้นมาก แปลว่าหน้าแดงผิดปกติ
                    elif a_diff > 4.5: 
                        current_skin_status = "Flushing (RED)"
                    else: 
                        current_skin_status = "Normal"

            # --- 2. Dynamic Polygon Drawing ---
            ...
            for img_to_draw in [clean_image, image]:
                ...
                if skin_calibrating:
                    ...
                    color = (255, 255, 255) if current_skin_status == "Pallor (Pale)" else (0, 0, 255) if current_skin_status == "Flushing (RED)" else (0, 255, 0)
                    cv2.putText(img_to_draw, f"Skin   : {current_skin_status}", (panel_x + 10, y_skin), cv2.FONT_HERSHEY_SIMPLEX, font_scale, color, 1)
